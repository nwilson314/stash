    ...
    async def process_new_link(self, url: str) -> LinkMetadata:
        ...
        try:
            ...
            r = await self.http_client.get(
                clean_url, follow_redirects=True, timeout=5.0
            )
            r.raise_for_status()
            ...
            # Detect content type
            content_type = self._detect_content_type(final_url, r.headers)
            ...
            if content_type == ContentType.YOUTUBE:
                ...
                if video_id:
                    ...
                    try:
                        oembed_response = await self.http_client.get(
                            oembed_url, timeout=3.0
                        )
            ...
            if (title is None) and "html" in r.headers.get("content-type", "").lower():
                ...
                og_match = re.search(
                    r'<meta\s+property="og:title"\s+content="([^"]+)"', html_content
                )
                if og_match:
                ...
                if author is None:
                    author_match = re.search(
                        r'<meta\s+name="author"\s+content="([^"]+)"', html_content
                    )
                    if author_match:
            ...
            return LinkMetadata(url=clean_url, error="Request timed out")
            ...
            return LinkMetadata(
                url=clean_url, error=f"HTTP error: {e.response.status_code}"
            )
